...
function createLoader() {
    ...
    return {
        ...
        /**
         * Load an indiviudal service by its file path
         *
         ...
         * @param modPath - the full path to the service
         * @param failOnDups - if specified, error out if there are name collisions with an existing service
         * @param [modId] - The desired module id; defaults to normalized module name if not provided
         */
        loadService: function (modPath, failOnDups, modId) {
            var mod;
            try {
            ...
            registerServiceModule(mod, failOnDups, modId);
        },
        ...
        loadServices: function (dir, failOnDups) {
            ...
            di.iterateOverJsFiles(dir, function(dir, file) {
                ...
                loader.loadService(modPath, failOnDups);
            });
        },

        /**
         * Load an individual consumer by its file path
         *
         * @param modPath - The file path to the module
         * @param prefix - The prefix to use when calling registerConsumerModule
         * @param [modId] - The desired module id; defaults to normalized module name if not provided
         */
        loadConsumer: function (modPath, prefix, _modId) {
            var mod;
            var modId = normalizeServiceName(_modId || path.parse(modPath).name);
            try {
                mod = require(modPath);
            } catch (e) {
                throw new Error('Could not load ' + modPath + ': ' + e.message);
        ...
        /**
         * Load all the consumers in a given directory.
         *
        ...
        loadConsumers: function (dir, prefix) {
            ...
            di.iterateOverJsFiles(dir, function(dir, file) {
                var modPath = path.resolve(dir, file);
                loader.loadConsumer(modPath, prefix);
            });
        },
    ...
    function registerServiceModule(mod, failOnDup, _modId) {

        // Allow a custom module name to be provided when registering. If provided, overwrite the module id.
        if (_modId) {
            mod.__id = _modId;
        }
